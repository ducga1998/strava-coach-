...
async def process_activity_metrics(session: AsyncSession, activity: Activity) -> None:
    ...
    # Autosync runs on its own session (see workers.tasks.enqueue_plan_sync) —
    # must NOT share this session, or a sync-time commit/rollback would
    # corrupt the in-flight activity state here.
    from app.workers.tasks import enqueue_plan_sync
    await enqueue_plan_sync(activity.athlete_id)
